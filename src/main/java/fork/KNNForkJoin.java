...
/**
 * @class KNNForkJoin
 * @brief Implements the K-Nearest Neighbors algorithm using the Fork/Join Framework (Version 3).
 * Mirrors the same fixed-partition strategy used by {@code KNNPlatform}: the file is split
 * into exactly {@code NUM_THREADS} line-aligned byte chunks, and each chunk is processed by
 * its own {@link RecursiveTask} instead of a raw platform {@link Thread}. A single root task
 * builds one leaf task per chunk and dispatches all of them at once via
 * {@link ForkJoinTask#invokeAll(Collection)} — which forks every leaf but the last, computes
 * the last one on the calling worker, then joins the rest — exactly the "fork the others,
 * compute the last, join everything" pattern described for {@code fork()}/{@code join()}.
 * Every leaf task returns its own local Top-K {@link PriorityQueue}, so results are merged
 * only after {@code join()}, with no shared mutable state and no {@code synchronized} blocks
 * anywhere in the parallel phase.
 */
public class KNNForkJoin {
     ...
     * Orchestrates execution by invoking the parallel engine with a ForkJoinPool
     * sized to the number of available processors, just like the fixed thread count
     * used by {@code KNNPlatform}.
    ...
    public String predictStream(String filePath, Neighbor target, int k) {
        System.out.printf("[ForkJoin] Using ForkJoinPool with %d workers%n", NUM_THREADS);
        return runParallel(filePath, target, k, NUM_THREADS);
    }

    /**
     * @method runParallel
     * @brief Manages the execution lifecycle of the Fork/Join-based workers.
     * Computes file positions exactly like {@code KNNPlatform}, submits the root
     * {@link RootTask} to a {@link ForkJoinPool} via {@code invoke()} — which internally
     * forks one {@link ChunkTask} per chunk, computes the last chunk on the calling worker,
     * and joins the rest — merges the individual local Top-K results, and triggers the
     * final majority vote.
     * @param filePath Path to the CSV dataset file.
     * @param target The target instance to be classified.
     * @param k The number of nearest neighbors to consider.
     * @param numThreads The size of the ForkJoinPool to deploy.
    ...
    private String runParallel(String filePath, Neighbor target, int k, int numThreads) {
        ...
        try {
            RootTask rootTask = new RootTask(filePath, chunks, target, k);
            globalTopK = pool.invoke(rootTask);
        } finally {
            pool.shutdown();
        }

        if (globalTopK == null || globalTopK.isEmpty()) {
    ...
    /**
     * @class RootTask
     * @brief RecursiveTask that owns the whole chunk list and dispatches one {@link ChunkTask}
     * per chunk via {@code invokeAll}, then merges every local Top-K into a single global one.
     * This is the direct Fork/Join analogue of the {@code for (i : chunks) thread[i].start()}
     * loop followed by {@code thread.join()} used in {@code KNNPlatform}.
     */
    private static class RootTask extends RecursiveTask<PriorityQueue<DistanceRecord>> {

        private final String filePath;
        private final List<ChunkBounds> chunks;
        private final Neighbor target;
        private final int k;

        RootTask(String filePath, List<ChunkBounds> chunks, Neighbor target, int k) {
        ...
        @Override
        protected PriorityQueue<DistanceRecord> compute() {
            List<ChunkTask> tasks = new ArrayList<>(chunks.size());
            for (ChunkBounds chunk : chunks) {
                tasks.add(new ChunkTask(filePath, chunk, target, k));
            }

            // Forks every task but the last, computes the last on this worker,
            // then joins all of them — the standard fork()/compute()/join() fan-out.
            ForkJoinTask.invokeAll(tasks);

            PriorityQueue<DistanceRecord> globalTopK = new PriorityQueue<>(k, Collections.reverseOrder());

            for (ChunkTask task : tasks) {
                PriorityQueue<DistanceRecord> localTopK = task.join();
    ...
    /**
     * @class ChunkTask
     * @brief RecursiveTask responsible for a single pre-computed chunk, mirroring exactly
     * the work a single platform {@link Thread} does in {@code KNNPlatform.processChunk}.
     * There is no further splitting here — each chunk is already sized for one worker,
     * exactly as in the fixed-partition versions.
     */
    private static class ChunkTask extends RecursiveTask<PriorityQueue<DistanceRecord>> {

        private final String filePath;
        private final ChunkBounds chunk;
        private final Neighbor target;
        private final int k;

        ChunkTask(String filePath, ChunkBounds chunk, Neighbor target, int k) {
            this.filePath = filePath;
            this.chunk = chunk;
            this.target = target;
            this.k = k;
     ...
     * when dividing the file among worker tasks. Identical to {@code KNNPlatform.computeChunks}.
    ...
    /**
     * @method processChunk
     * @brief Processes a specific chunk of the file assigned to a single ChunkTask.
